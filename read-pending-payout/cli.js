...
async function requestSideCar(url) {
...
async function main() {
  ...
  if (!accountId) {
    ...
    const res = await requestSideCar(`${sideCarUrl}/blocks/head`);
  ...
  const stakingPayouts = await requestSideCar(
  ...
  if (unclaimedOnly) {
    console.log(
      `Total payout unclaimed: ${formatPayout(unclaimedTotalPayout)}`
    );
  } else {
    console.log(`Total payout: ${formatPayout(totalPayout)}`);
    console.log(`Claimed: ${formatPayout(claimedTotalPayout)} has been`);
    console.log(`Unclaimed: ${formatPayout(unclaimedTotalPayout)}`);
  }
}
